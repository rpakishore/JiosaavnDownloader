...
class JiosaavnDownload:
    ...
    def __init__(
        self, cache_filepath: str | None = None, final_location: Path | None = None
    ) -> None:
        config = get_config()
        if cache_filepath is None:
            cache_filepath = (
                Path(config.get("paths", {}).get("db_folder", ".")) / "database.pkl"
            )
    ...
    def song(self, url: str, skip_downloaded: bool = True, debug_only: bool = False):
        self._download_song(
            song=self.ApiProvider.song(url=url),
    ...
    def _download_song(self, song: Song, skip_downloaded: bool, debug_only: bool):
        with song:
            ...
            if skip_downloaded and self.check_downloaded(song):
                log.debug(
                    f"Skipping {song.sanitized_name} from {song.sanitized_album}, Downloaded on {song.download_date}"
                )
                return
    ...
    def __gotify(self, song: Song) -> None:
        ...
        notify(
            app=self.GOTIFY_CHANNEL,
            title=_title,
            message=_msg,
            priority=2,
            url=self.GOTIFY_URL,
            apptoken=self.__gotify_key,
        )

    def search_songs(
        self, query: str, skip_downloaded: bool = True, debug_only: bool = False
    ):
        songs = self.ApiProvider.search_song(query=query)
        for song in songs:
            self._download_song(
                song=song, skip_downloaded=skip_downloaded, debug_only=debug_only
            )

    def playlist(
        self, id_link: str | int, skip_downloaded: bool = True, debug_only: bool = False
    ):
        if isinstance(id_link, str) and (
            id_link.startswith("http") or id_link.startswith("www")
        ):
